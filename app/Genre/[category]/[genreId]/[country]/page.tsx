...
export default function Genre() {
     ...
     const { genreId , category , country  } = useParams();
     

     const [movie , setMovie] = useState<GenreMovies>();
   ...
   const res = await fetch(`https://api.themoviedb.org/3/discover/${category}?with_genres=${genreId}&language=en-US&with_origin_country=${country}&page=${currentPage}`, {
      headers: {
...
const searchParams = useSearchParams();
       ...
       const currentPage = Number(searchParams.get("page")) || 1;
         ...
         const totalPages = Math.min(movie?.total_pages ?? 1, 500);
         const pageLimit = 6; 
  ...
  return (
    ...
    <div className='p-5 lg:px-20 '>
      ...
      <SideBar category ={category} genreId = {genreId} />

     

      {movie?.results.map((ele)=>{

        return <div key={ele.id} className="rounded-2xl border-2 shadow-2xs flex my-6 mr-5 items-start hover:border-blue-900 hover:shadow-xl transition-all duration-300">
