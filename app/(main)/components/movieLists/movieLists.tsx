...
export default function MovieListss() {


    let [movie , setMovie] = useState<MovieLists>()

    let [category , setCategory] = useState<"popular" | "top_rated" | "now_playing">("popular")



     async function getMoviesLists(type:"popular" | "top_rated" | "now_playing") {

       const res = await fetch(`https://api.themoviedb.org/3/movie/${type}?language=en-US&page=1`, {
        headers: {
  ...
  <Tabs
    ...
    value={category}
    onValueChange={(v) =>
      setCategory(v as "popular" | "top_rated" | "now_playing")
    }
  >
    <TabsList
      className="
    ...
    >
      <TabsTrigger
        value="popular"
        className="
          rounded-3xl  px-2 md:px-6 py-2 mx-1 text-black 
      ...
      <TabsTrigger
        value="top_rated"
        className="
          rounded-3xl  px-2 md:px-6 py-2  text-black md:mx-2 mx-1
          hover:bg-blue-950 hover:text-yellow-400 
          data-[state=active]:bg-blue-950 data-[state=active]:text-yellow-400
          flex-1 sm:flex-none
        "
...
<div className="absolute bg-gray-800 rounded-full left-4 -bottom-5 w-10 h-10 text-white">
  <svg viewBox="0 0 36 36" className="w-full h-full">
    ...
    >
      ...
      <div>
    <h3 className='text-black mt-5'>{ele.original_title || ele.title || ele.original_title}</h3>
    <span className='text-gray-500 font-light'>{ele.release_date ? new Date(ele.release_date).toDateString() :  "" }</span>
</div>
    </CarouselItem>
